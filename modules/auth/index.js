...
const { hashMd5, uuidv4, signToken, getRandomPassword } = require("../utils");
...
const handlers = {
  ...
  async login(req, res, next) {
    try {
      ...
      let user = await User.findOne({ email: formattedEmail }).populate(
        populateQuery
      );
  ...
  async changeName(req, res, next) {
    try {
      ...
      await User.updateOne({ _id: req.user.id }, { $set: { name: newName } });
